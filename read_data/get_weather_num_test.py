...
import get_weather_num_test as gwn
...
def get_weather_num(fn):
	...
	df=pd.read_table('weather_data/weather_data_2016-01-'+filen2s(fn)+'_test',header=None,names=['Time','Weather','temperature','PM2.5'])

	df['Time']=df['Time'].map(time_slice)	
	sdf=df.sort(['Time'])
	sdf.to_csv('traffic_weather_data/weather_data_2016-01-'+filen2s(fn)+'_test.csv',columns=['Time','Weather','temperature','PM2.5'],index=False)	
'''
#Too slow!
