...
sys.path.append("f:/di____di/season_1/training_data/t_weather_data")

import get_traffic_num as gtn

import os
os.chdir('f:/di____di/season_1/training_data/t_weather_data')
'''
df=pd.read_csv('weather_data_2016-01-01.csv')
Time  Weather  temperature  PM2.5

gb=df['Time','Weather','temperature','PM2.5'].groupby(['Time']).mean()
import os
import string
import pandas as pd
import numpy as np
...
def get_traffic_num(fn):		
	...
	sdf.to_csv('traffic_num_data/traffic_num_data_2016-01-'+filen2s(fn)+'.csv',columns=['district_id','tj_time_slice','tj_time','tj_level1','tj_level2','tj_level3','tj_level4'],index=False);
