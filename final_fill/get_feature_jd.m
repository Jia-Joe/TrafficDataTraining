...
X_train=-1000*ones(66*97*20,28);
y_train=zeros(66*97*20,1);

mean_mx=zeros(66,144);
mean_item=zeros(66*97*20,1);
for i=1:66   
    for j=1:144 
        sum=0;
        for k=2:21     
            index=144*21*(i-1)+21*(j-1)+k;
            sum=sum+copy(index,item);
        end
        mean_mx(i,j)=sum/20;
    end
end
...
for i=1:66
    for j=1:97
        for k=1:20
            mean_item((i-1)*97*20+(j-1)*20+k,1)=mean_mx(i,45+j);
        end
    end
end
